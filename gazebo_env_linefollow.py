...
class Gazebo_Linefollow_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
		...
		# Binary threshold all blue pixels
        _, binary = cv2.threshold(cv_image[:,:,2], 100, 255, cv2.THRESH_BINARY_INV)

        contours, _ = cv2.findContours(binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        if not contours:
            self.timeout += 1
            done = self.timeout >= MAX_TIMEOUT
            return state, done
